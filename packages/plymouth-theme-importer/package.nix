# Source: https://github.com/NixOS/nixpkgs/tree/master/pkgs/by-name/ad/adi1090x-plymouth-themes
{
  stdenv,
  fetchurl,
  lib,
...
let
  ...
  customThemeShas = import ./extra-shas.nix;
  allThemeShas = themeShas // customThemeShas;

  knownThemes = builtins.attrNames allThemeShas;
  selectedThemes =
  ...
  srcs = lib.lists.forEach selectedThemes (
    name:
    (fetchurl {
      url = allThemeShas.${name}.url;
      sha256 = allThemeShas.${name}.sha;
    })
  );
in
stdenv.mkDerivation {
