{
  ...
  bisq-launcher = args:
    writeShellScript "bisq-launcher" ''
      ...
      # Extract the exact classpath intended by upstream jpackage
      CFG_CLASSPATH=$(grep '^app.classpath=' @out@/lib/app/*.cfg | head -n 1 | cut -d= -f2)

      # Strip potential prefixes ($APPDIR/ or lib/app/) depending on the jpackage version
      CLEAN_CP=$(echo "$CFG_CLASSPATH" | sed 's|$APPDIR/||g; s|lib/app/||g')

      # Rebuild the classpath string using absolute Nix store paths
      FULL_CP=$(echo "$CLEAN_CP" | sed -e 's|:|:@out@/lib/app/|g' -e 's|^|@out@/lib/app/|')

      exec "${lib.getExe jdk}" -Djpackage.app-version=@version@ -classpath "$FULL_CP" ${args} bisq.desktop_app_launcher.DesktopAppLauncher "$@"
    '';
